<template>
  <div>
    <router-link v-if="isUserAuth" to="/personal/reservations" class="auth-link">
      <avatar
        v-if="!profileSidebarInfo.avatar.avatar_file_name && !isUserIncomplete"
        class="auth-link__image mr-8"
        :class="{
          update: profileSidebarInfo.isUpdate,
          'is-gold': profileSidebarInfo.partnerStatus.includes('gold'),
        }"
        :username="profileSidebarInfo.firstName + ' ' + profileSidebarInfo.lastName"
        :size="32"
      />
      <span v-else class="auth-link__image mr-8">
        <img v-if="isUserIncomplete" src="~/assets/img/avatar.svg" alt="avatar" />
        <img
          v-else
          :class="{
            update: profileSidebarInfo.isUpdate,
            'is-gold': profileSidebarInfo.partnerStatus.includes('gold'),
          }"
          :src="renderedPhoto"
          alt="avatar"
        />
      </span>
      <span v-if="userProfile" class="auth-link__text fs-14 fw-medium">
        {{ isUserIncomplete ? '' : userProfile.firstname + ' ' + userProfile.lastname }}
      </span>
      <span v-else class="auth-link__text fs-14 fw-medium">
        {{ $t('lk__lk.translation') }}
      </span>
    </router-link>

    <template v-else>
      <a href="#" class="auth-link" @click.prevent="$bvModal.show('modal-sign-in')">
        <span class="auth-link__image">
          <img src="~/assets/img/avatar.svg" alt="avatar" />
        </span>
        <span class="auth-link__text fs-14 fw-medium m-hidden ml-8">
          {{ $t('lk_.translation') }}
        </span>
      </a>
    </template>
  </div>
</template>

<script>
import Avatar from 'vue-avatar';
import { mapGetters, mapState } from 'vuex';
import { getAbsoluteImgPath } from '@/helpers/pictures';

export default {
  components: {
    Avatar,
  },
  computed: {
    ...mapGetters('user', ['isUserAuth']),
    userProfile() {
      return this.$store.state.user.userProfile;
    },
    isUserIncomplete() {
      return this.userProfile.roles?.includes('ROLE_INCOMPLETE');
    },
    ...mapState('user', {
      profileSidebarInfo: (state) => state.profileSidebarInfo,
    }),
    renderedPhoto() {
      if (this.profileSidebarInfo.avatar.avatar_file_name) {
        return getAbsoluteImgPath(this.profileSidebarInfo.avatar.avatar_file_name);
      } else {
        return '';
      }
    },
  },
};
</script>

<style lang="scss">
@import './auth.scss';
</style>
